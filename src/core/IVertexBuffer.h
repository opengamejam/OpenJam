...
struct IVertexBuffer::SVertexStream {
    ...
    {
        return (dataType != Unknown && stride != 0);
    }
    ...
    void CopyFrom(const SVertexStream& srcStream, uint64_t srcIndex, uint64_t size, uint64_t dstIndex)
    {
        ...
        if (!IsActive() || !srcStream.IsActive() || !dstVB || !srcVB || dstIndex + size > dstVB->Size() || srcIndex + size > srcVB->Size()) {
            return;
        }
    ...
    INL void Set(uint64_t startIndex, const std::vector<T>& srcData)
    {
        ...
        if (!IsActive() || !vb || startIndex >= vb->Size()) {
            return;
        }
...
INL bool CompareCapability(IVertexBufferPtr vb1, IVertexBufferPtr vb2)
{
    ...
    return (stream1.size() == stream2.size() && std::equal(stream1.begin(), stream1.end(), stream2.begin(), pred));
}
