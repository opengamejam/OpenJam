...
bool CTextureOGLES1::IsValid() const
{
    return (m_Id != 0);
}

void CTextureOGLES1::Filter(ITexture::TextureFilters filter)
{
    if (!IsValid())
    {
        return;
...
ITexture::TextureFilters CTextureOGLES1::Filter() const
{
    return m_Filter;
}

bool CTextureOGLES1::AssignImage(IImagePtr image)
{
    if (!IsValid())
    {
...
const std::string& CTextureOGLES1::Hash()
{
    if (m_IsDirty)
    {
        HashMe();
