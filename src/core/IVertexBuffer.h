//
//  ISprite.h
//  OpenJam
//
//  Created by yev
//  Copyright (c) 2014 Yevgeniy Logachev. All rights reserved.
//
#ifndef IVERTEXBUFFER_H
#define IVERTEXBUFFER_H

#include "Global.h"

namespace jam
{
    
CLASS_PTR(IVertexBuffer);
CLASS_WEAK(IVertexBuffer);
    
class IVertexBuffer : public std::enable_shared_from_this<IVertexBuffer>
{
public:
    enum DataTypes
    {
        Unknown = -1,
        Byte = 0,
        UByte,
        Short,
        UShort,
        Int,
        UInt,
        Float,
        ShortFloat,
    };
    
    enum VertexTypes
    {
        Position,
        TextureCoords,
        Normal,
        Color,
        Tangent
    };
    
    struct SVertexStream
    {
        IVertexBufferWeak vertexBuffer;
        unsigned char offset;
        unsigned char stride;
        DataTypes dataType;
        int attributeIndex;
        unsigned int streamIndex;
        unsigned int absoluteOffset; // Used for zero-stride mode
        
        SVertexStream(IVertexBufferWeak _vertexBuffer)
        : vertexBuffer(_vertexBuffer)
        , offset(0)
        , stride(0)
        , dataType(Unknown)
        , attributeIndex(0)
        , streamIndex(0)
        , absoluteOffset(0)
        {
        
        }
        
        SVertexStream()
        : vertexBuffer(IVertexBufferWeak())
        , offset(0)
		, stride(0)
		, dataType(Unknown)
		, attributeIndex(0)
		, streamIndex(0)
		, absoluteOffset(0)
        {
        
        }
        
        INL bool IsActive() const
        {
            return (dataType != Unknown &&
                    stride != 0);
        }
        
        template <class T>
        INL void Set(uint64_t startIndex, const std::vector<T>& array)
        {
            IVertexBufferPtr vb = vertexBuffer.lock();
            if (!vb && IsActive())
            {
                return;
            }
            
            bool needRestoreLock = !vb->IsLocked();
            char* dst = static_cast<char*>(vb->LockRaw());
            const char* src = reinterpret_cast<const char*>(array.data());
            
            uint64_t srcElemSize = sizeof(T);
            uint64_t dstElemSize = sizeForType(dataType);
            uint64_t sizeElemToCopy = dstElemSize * stride;
            
            uint64_t srcSize = array.size() * srcElemSize;
            uint64_t dstSize = (vb->Size() - startIndex) * sizeElemToCopy;
            uint64_t sizeToCopy = std::min(srcSize, dstSize);
            
            if (vb->ZeroStride())
            {
                memcpy(dst + absoluteOffset, src, sizeToCopy);
            }
            else
            {
                uint64_t srcIndex = 0;
                uint64_t dstIndex = 0;
                uint64_t num = sizeToCopy / (dstElemSize * stride);
                for (uint64_t i = 0; i < num; ++i)
                {
                    dstIndex = (startIndex + i) * vb->ElementSize() + offset;
                    srcIndex = i * sizeElemToCopy;
                    
                    memcpy(&dst[dstIndex], &src[srcIndex], sizeElemToCopy);
                }
            }
            
            if (needRestoreLock)
            {
                vb->Unlock(true);
            }
        }
        
        template <class T>
        INL void Get(uint64_t startIndex, std::vector<T>& array)
        {
            IVertexBufferPtr vb = vertexBuffer.lock();
            if (!vb && IsActive())
            {
                return;
            }
            
            bool needRestoreLock = !vb->IsLocked();
            char* dst = static_cast<char*>(vb->LockRaw());
            const char* src = reinterpret_cast<const char*>(array.data());
            
            uint64_t dstElemSize = sizeof(T);
            uint64_t srcElemSize = sizeForType(dataType);
            uint64_t sizeElemToCopy = srcElemSize * stride;
            
            uint64_t sizeToCopy = (vb->Size() - startIndex) * srcElemSize * stride;
            
            if (vb->ZeroStride())
            {
                memcpy(dst + absoluteOffset, src, sizeToCopy);
            }
            else
            {
                uint64_t srcIndex = 0;
                uint64_t dstIndex = 0;
                uint64_t num = sizeToCopy / (dstElemSize * stride);
                for (uint64_t i = 0; i < num; ++i)
                {
                    dstIndex = (startIndex + i) * vb->ElementSize() + offset;
                    srcIndex = i * sizeElemToCopy;
                    
                    memcpy(&dst[dstIndex], &src[srcIndex], sizeElemToCopy);
                }
            }
            
            if (needRestoreLock)
            {
                vb->Unlock();
            }
        }
        
        template <class T>
        INL void Set(uint64_t index, const T& inValue)
        {
            IVertexBufferPtr vb = vertexBuffer.lock();
            if (!vb && IsActive())
            {
                return;
            }
            
            uint64_t offset = index * sizeof(T);
            if (offset + sizeof(T) >= vb->SizeRaw())
            {
                return;
            }
            
            bool needRestoreLock = !vb->IsLocked();
            T* dst = static_cast<T*>(vb->LockRaw());
            *dst = inValue;
            
            if (needRestoreLock)
            {
                vb->Unlock(true);
            }
        }
        
        template <class T>
        INL void Get(uint64_t index, T& outValue)
        {
            IVertexBufferPtr vb = vertexBuffer.lock();
            if (!vb && IsActive())
            {
                return;
            }
            
            uint64_t offset = index * sizeof(T);
            if (offset + sizeof(T) >= vb->SizeRaw())
            {
                return;
            }
            
            bool needRestoreLock = !vb->IsLocked();
            T* dst = static_cast<T*>(vb->LockRaw());
            outValue = *dst;
            
            if (needRestoreLock)
            {
                vb->Unlock();
            }
        }
        
        size_t DataSize() const
        {
            return sizeForType(dataType);
        }
        
        static size_t sizeForType(DataTypes type)
        {
            static std::map<DataTypes, size_t> types = {
                {Unknown, 0},
                {Byte, sizeof(char)},
                {UByte, sizeof(unsigned char)},
                {Short, sizeof(short)},
                {UShort, sizeof(unsigned short)},
                {Int, sizeof(int)},
                {UInt, sizeof(unsigned int)},
                {Float, sizeof(float)},
                {ShortFloat, 2}, // TODO:
            };
            
            return types[type];
        }
    };
    
    typedef std::map<VertexTypes, SVertexStream> TVertexStreamMap;
    
public:
    IVertexBuffer() = default;
    virtual ~IVertexBuffer() = default;
    
    virtual void Initialize(size_t elementSize) = 0;
    virtual void Shutdown() = 0;
    virtual bool IsValid() const = 0;
    
    virtual size_t ElementSize() const = 0;
    
    virtual size_t SizeRaw() const = 0;
    size_t Size()
    {
        if (SizeRaw() % ElementSize() == 0)
        {
            return (SizeRaw() / ElementSize());
        }
        return 0;
    }
    
    virtual void ResizeRaw(size_t newSize) = 0;
    void Resize(size_t newSize)
    {
        ResizeRaw(newSize * ElementSize());
    }
    
    virtual void* LockRaw() = 0;
    virtual SVertexStream& Lock(VertexTypes vertexType) = 0;
    virtual bool IsLocked() const = 0;
    virtual void Unlock(bool isNeedCommit = false) = 0;
    virtual bool HasStream(VertexTypes vertexType) = 0;
    
    virtual void ZeroStride(bool isZeroStride) = 0;
    virtual bool ZeroStride() = 0;
    
    virtual const TVertexStreamMap& VertexStreams() const = 0;
    
    virtual void Bind() = 0;
    virtual void Unbind() = 0;
    
protected:
    virtual void ElementSize(size_t elementSize) = 0;
};
    
}; // namespace jam;

#endif /* IVERTEXBUFFER_H */
