 ...
 */
float mvkMTLPixelFormatSizeInBytes(MTLPixelFormat mtlFormat);

/**
 * Returns the size, in bytes, of a texel of the specified Vulkan format.
 * The returned value may be fractional for certain compressed formats.
 */
float mvkVkFormatSizeInBytes(VkFormat vkFormat);
 ...
 *
 * The isMultsample arguments are ignored unless vkImageType is VK_IMAGE_TYPE_2D.
 */
...
MTLTextureType mvkMTLTextureTypeFromVkImageViewType(VkImageViewType vkImageViewType);
...
#define MVK_VK_MEMORY_TYPE_METAL_MANAGED	(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
