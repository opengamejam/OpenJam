...
CFrameBufferTargetOGLES2::CFrameBufferTargetOGLES2(unsigned int width, unsigned int height)
: m_FrameBuffer(-1)
, m_DepthBuffer(-1)
, m_StencilBuffer(-1)
...
CFrameBufferTargetOGLES2::~CFrameBufferTargetOGLES2()
{
    unsigned int i = 0;
    std::for_each(m_ColorBuffers.begin(), m_ColorBuffers.end(), [&](unsigned int colorBuffer)
    {
...
void CFrameBufferTargetOGLES2::Initialize(unsigned int externalFrameBuffer, unsigned int externalColorBuffer,
...
void CFrameBufferTargetOGLES2::Initialize()
{
    if (m_FrameBuffer == -1)
    {
...
bool CFrameBufferTargetOGLES2::CreateColorAttachment(int index)
{
...
bool CFrameBufferTargetOGLES2::CreateDepthAttachment()
{
    if (m_DepthBuffer != -1)
    {
        return true;
    ...
    xglFramebufferRenderbuffer(XGL_FRAMEBUFFER, XGL_STENCIL_ATTACHMENT, XGL_RENDERBUFFER, m_DepthBuffer);
...
bool CFrameBufferTargetOGLES2::CreateStencilAttachment()
{
    if (m_StencilBuffer != -1)
    {
        return true;
...
bool CFrameBufferTargetOGLES2::CreateTextureAttachment()
{
    return false;   // TODO:
}

void CFrameBufferTargetOGLES2::Bind() const
{
    xglBindFramebuffer(XGL_FRAMEBUFFER, m_FrameBuffer);
}

void CFrameBufferTargetOGLES2::Unbind() const
{
    xglBindFramebuffer(XGL_FRAMEBUFFER, -1);
}

void CFrameBufferTargetOGLES2::Clear() const
{
...
void CFrameBufferTargetOGLES2::ClearColor(const CColor& color)
{
    m_ClearColor = color;
}

const CColor& CFrameBufferTargetOGLES2::ClearColor() const
{
    return m_ClearColor;
}

unsigned int CFrameBufferTargetOGLES2::Width() const
{
    return m_Width;
}

unsigned int CFrameBufferTargetOGLES2::Height() const
{
    return m_Height;
}

std::vector<unsigned char> CFrameBufferTargetOGLES2::RawData()
{
    Bind();
